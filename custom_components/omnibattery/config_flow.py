...
class MarstekVenusConfigFlow(LegacyDomainMigrationMixin, ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_predictive_charging_config(
        ...
        schema_dict[vol.Optional(CONF_PREDICTIVE_SAFETY_MARGIN_KWH, default=default_predictive_safety_margin_kwh(self.config_data))] = NumberSelector(
    ...
    async def async_step_dynamic_pricing_config(
                ...
                if not errors:
                    ...
                    if max_price is not None and discharge_price is not None and discharge_price < max_price:
                        ...
                        self.config_data[CONF_HIGH_PRICE_DISCHARGE_MAX_POWER] = user_input.get(
                            CONF_HIGH_PRICE_DISCHARGE_MAX_POWER,
                            default_high_price_discharge_max_power(self.config_data),
                        )
        ...
        schema_dict[vol.Optional(CONF_HIGH_PRICE_DISCHARGE_MAX_POWER, default=default_high_price_discharge_max_power(self.config_data))] = NumberSelector(
...
class OptionsFlowHandler(OptionsFlow):
    ...
    async def async_step_predictive_charging_config(
        ...
        defaults = {
            ...
            "margin": existing_config.get(CONF_PREDICTIVE_SAFETY_MARGIN_KWH, default_predictive_safety_margin_kwh(existing_config)),
        }
    ...
    async def async_step_dynamic_pricing_config(
                ...
                if not errors:
                    ...
                    if max_price is not None and discharge_price is not None and discharge_price < max_price:
                        ...
                        self.config_data[CONF_HIGH_PRICE_DISCHARGE_MAX_POWER] = user_input.get(
                            ...
                            existing_config.get(
                                CONF_HIGH_PRICE_DISCHARGE_MAX_POWER,
                                default_high_price_discharge_max_power(existing_config),
                            ),
                        )
        ...
        default_margin = existing_config.get(CONF_PREDICTIVE_SAFETY_MARGIN_KWH, default_predictive_safety_margin_kwh(existing_config))
        ...
        default_high_price_discharge_power_w = existing_config.get(
            CONF_HIGH_PRICE_DISCHARGE_MAX_POWER,
            default_high_price_discharge_max_power(existing_config),
        )
        ...
        schema_dict[vol.Optional(CONF_HIGH_PRICE_DISCHARGE_MAX_POWER, default=default_high_price_discharge_power_w)] = NumberSelector(
            NumberSelectorConfig(min=0, max=10000, step=50, unit_of_measurement="W", mode=NumberSelectorMode.BOX)
        )
