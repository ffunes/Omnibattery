...
const I18N = {
  ...
  es: {
    ...
    secHourly: "Balance horario", hourlyEsOnly: "Solo útil en España (RD 244/2019) · país detectado: {c}", secWeeklyFull: "Carga semanal completa", itemWeeklyDay: "Día de carga completa", itemWeeklyDelay: "Esperar al retraso por solar", itemWeeklyGrid: "Cargar desde la red", itemWeeklyGridMode: "Modo de carga desde red", itemWeeklyGridPowerPct: "Potencia de carga desde red", itemHourlyTarget: "Objetivo de balance neto", itemHourlyMaxOffset: "Offset máx. de potencia", itemHourlyDeadband: "Banda muerta", itemHourlyHysteresis: "Histéresis",
  ...
  ca: {
    ...
    secHourly: "Balanç horari", hourlyEsOnly: "Només útil a Espanya (RD 244/2019) · país detectat: {c}", secWeeklyFull: "Càrrega setmanal completa", itemWeeklyDay: "Dia de càrrega completa", itemWeeklyDelay: "Espera el retard per solar", itemWeeklyGrid: "Carregar des de la xarxa", itemWeeklyGridMode: "Mode de càrrega des de xarxa", itemWeeklyGridPowerPct: "Potència de càrrega des de xarxa", itemHourlyTarget: "Objectiu de balanç net", itemHourlyMaxOffset: "Offset màx. de potència", itemHourlyDeadband: "Banda morta", itemHourlyHysteresis: "Histèresi",
  ...
  de: {
    ...
    secHourly: "Stündliche Balance", hourlyEsOnly: "Nur in Spanien sinnvoll (RD 244/2019) · erkanntes Land: {c}", secWeeklyFull: "Wöchentliche Vollladung", itemWeeklyDay: "Tag der Vollladung", itemWeeklyDelay: "Auf Solar-Ladeverzögerung warten", itemWeeklyGrid: "Aus dem Netz laden", itemWeeklyGridMode: "Netzladungsmodus", itemWeeklyGridPowerPct: "Netzladungsleistung", itemHourlyTarget: "Ziel-Nettobalance", itemHourlyMaxOffset: "Max. Leistungs-Offset", itemHourlyDeadband: "Totband", itemHourlyHysteresis: "Hysterese",
  ...
  nl: {
    ...
    secHourly: "Uurbalans", hourlyEsOnly: "Alleen nuttig in Spanje (RD 244/2019) · gedetecteerd land: {c}", secWeeklyFull: "Wekelijkse volledige lading", itemWeeklyDay: "Dag volledige lading", itemWeeklyDelay: "Wachten op zonne-laadvertraging", itemWeeklyGrid: "Laden vanaf net", itemWeeklyGridMode: "Netlaadmodus", itemWeeklyGridPowerPct: "Netlaadvermogen", itemHourlyTarget: "Doel nettosaldo", itemHourlyMaxOffset: "Max. vermogensoffset", itemHourlyDeadband: "Dodeband", itemHourlyHysteresis: "Hysterese",
...
const SYS_SECTIONS = [
  ...
  },
  {
    ...
    items: [
      ...
      { key: "weekly_full_charge_grid", domain: "switch", lk: "itemWeeklyGrid", icon: "mdi:transmission-tower" },
      { key: "weekly_full_charge_grid_mode", domain: "select", lk: "itemWeeklyGridMode", icon: "mdi:transmission-tower" },
      { key: "weekly_full_charge_grid_power_pct", domain: "number", lk: "itemWeeklyGridPowerPct", icon: "mdi:speedometer" },
...
const SYS_HELP = {
  en: {
    ...
    weekly_full_charge_grid: "When ON, the integration imports from the grid on the weekly full charge day until every battery reaches 100%.",
    weekly_full_charge_grid_mode: "Immediate starts grid import as soon as the weekly cycle is active. Solar first waits until the solar forecast says the sun cannot cover the remaining energy to 100%.",
    weekly_full_charge_grid_power_pct: "Limits weekly grid charging to this percentage of each battery's max charge power (and the system charge cap if set). 100% = full speed.",
  ...
  es: {
    ...
    weekly_full_charge_grid: "Si está activado, la integración importa de la red el día de carga semanal completa hasta que todas las baterías lleguen al 100%.",
    weekly_full_charge_grid_mode: "Inmediato inicia la importación de red en cuanto el ciclo semanal está activo. Solar primero espera hasta que la previsión solar indique que el sol no puede cubrir la energía restante hasta el 100%.",
    weekly_full_charge_grid_power_pct: "Limita la carga semanal desde red a este porcentaje de la potencia máxima de carga de cada batería (y del límite del sistema si está configurado). 100% = velocidad completa.",
  ...
  ca: {
    ...
    weekly_full_charge_grid: "Si està activat, la integració importa de la xarxa el dia de càrrega setmanal completa fins que totes les bateries arribin al 100%.",
    weekly_full_charge_grid_mode: "Immediat inicia la importació de xarxa tan aviat com el cicle setmanal està actiu. Solar primer espera fins que la previsió solar indiqui que el sol no pot cobrir l'energia restant fins al 100%.",
    weekly_full_charge_grid_power_pct: "Limita la càrrega setmanal des de xarxa a aquest percentatge de la potència màxima de càrrega de cada bateria (i del límit del sistema si està configurat). 100% = velocitat completa.",
  ...
  de: {
    ...
    weekly_full_charge_grid: "Wenn EIN, importiert die Integration am Tag der wöchentlichen Vollladung aus dem Netz, bis jede Batterie 100% erreicht.",
    weekly_full_charge_grid_mode: "Sofort startet den Netzimport, sobald der wöchentliche Zyklus aktiv ist. Solar zuerst wartet, bis die Solarprognose sagt, dass die Sonne die verbleibende Energie bis 100% nicht decken kann.",
    weekly_full_charge_grid_power_pct: "Begrenzt die wöchentliche Netzladung auf diesen Prozentsatz der maximalen Ladeleistung jeder Batterie (und des Systemlimits, falls gesetzt). 100% = volle Geschwindigkeit.",
  ...
  fr: {
    ...
    weekly_full_charge_grid: "Si activé, l'intégration importe du réseau le jour de charge complète hebdomadaire jusqu'à ce que chaque batterie atteigne 100%.",
    weekly_full_charge_grid_mode: "Immédiat démarre l'import réseau dès que le cycle hebdomadaire est actif. Solaire d'abord attend que la prévision solaire indique que le soleil ne peut pas couvrir l'énergie restante jusqu'à 100%.",
    weekly_full_charge_grid_power_pct: "Limite la charge réseau hebdomadaire à ce pourcentage de la puissance de charge max. de chaque batterie (et du plafond système si défini). 100% = vitesse maximale.",
  ...
  nl: {
    ...
    weekly_full_charge_grid: "Indien AAN importeert de integratie op de dag van wekelijkse volledige lading van het net tot elke batterij 100% bereikt.",
    weekly_full_charge_grid_mode: "Onmiddellijk start netimport zodra de wekelijkse cyclus actief is. Zon eerst wacht tot de zonnevoorspelling aangeeft dat de zon de resterende energie tot 100% niet kan dekken.",
    weekly_full_charge_grid_power_pct: "Beperkt wekelijks netladen tot dit percentage van het maximale laadvermogen per batterij (en de systeemlimiet indien ingesteld). 100% = volle snelheid.",
