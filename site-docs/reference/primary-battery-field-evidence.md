...
Measurements from a mixed AC/DC installation, recorded on 26 and 27 August
2026 either side of a change made to the experimental build running on it.

**None of the control-layer behaviour described here is part of Omnibattery.**
The charge order, the scarcity threshold and the primary-battery selection were
prototyped on a fork (#335) and are being reimplemented in `control/`. What
follows is therefore evidence rather than documentation: it records what this
hardware did under a particular rule, so that an implementation can be judged
against measurements instead of against a description. Where a rule is named it
is the rule the build under test applied, not one the project ships.

The hardware behaviour is a different matter and does hold generally — a hybrid
inverter's response to a forcible command, and what a second battery looks like
to an energy manager, are properties of the equipment.
...
2. The hybrid refuses forcible charge/discharge commands while its strings
   carry voltage. The build under test reduces every daylight command to 0 for
   that reason (proposed in #381), and the inverter has been observed rejecting
...
## The rule under test, and how it failed

The build decided whether the day's sun was worth concentrating in one battery.
On a day judged scarce it put the DC-coupled battery first, which is sound:
kilowatt-hours stored without an AC conversion lose about 10 % less.

The threshold it used was the room left in the **whole fleet** — that is, "is
there enough sun to fill everything". On a mixed installation there almost never
is, so the verdict was scarce essentially every day and the AC-coupled battery
was passed over on all of them.

This is the part worth carrying forward, because the failure is in the question
rather than in the code that answers it.
...
The build was changed to measure scarcity against the room in the battery the
order concentrates into — the DC-coupled one. Past that point the excess has to
be shared out regardless, so the day stops counting as scarce exactly where the
preference stops paying.
...
A second property fell out of measuring against the hybrid's room: as the
hybrid fills, that room shrinks toward zero, the day turns ample, and the order
hands over on its own. Nothing had to command the hybrid to stand down — which
